 ...
 * <br />
 * This sample has been ported from the NVIDIA CUSPARSE documentation example.
 */
public class JCusparseSample{
	public static void main(String args[]) {
		...
		cooRowIndexHostPtr[0] = 0;
		cooColIndexHostPtr[0] = 0;
		cooValHostPtr[0] = 1.0f;
		cooRowIndexHostPtr[1] = 0;
		cooColIndexHostPtr[1] = 2;
		cooValHostPtr[1] = 2.0f;
		cooRowIndexHostPtr[2] = 0;
		cooColIndexHostPtr[2] = 3;
		cooValHostPtr[2] = 3.0f;
		cooRowIndexHostPtr[3] = 1;
		cooColIndexHostPtr[3] = 1;
		cooValHostPtr[3] = 4.0f;
		cooRowIndexHostPtr[4] = 2;
		cooColIndexHostPtr[4] = 0;
		cooValHostPtr[4] = 5.0f;
		cooRowIndexHostPtr[5] = 2;
		cooColIndexHostPtr[5] = 2;
		cooValHostPtr[5] = 6.0f;
		cooRowIndexHostPtr[6] = 2;
		cooColIndexHostPtr[6] = 3;
		cooValHostPtr[6] = 7.0f;
		cooRowIndexHostPtr[7] = 3;
		cooColIndexHostPtr[7] = 1;
		cooValHostPtr[7] = 8.0f;
		cooRowIndexHostPtr[8] = 3;
		cooColIndexHostPtr[8] = 3;
		cooValHostPtr[8] = 9.0f;

		// Print the matrix
		System.out.printf("Input data:\n");
		for (i = 0; i < nnz; i++) {
			System.out.printf("cooRowIndedHostPtr[%d]=%d  ", i,
					cooRowIndexHostPtr[i]);
			System.out.printf("cooColIndedHostPtr[%d]=%d  ", i,
					cooColIndexHostPtr[i]);
		...
		yHostPtr[0] = 10.0f;
		xIndHostPtr[0] = 0;
		xValHostPtr[0] = 100.0f;
		yHostPtr[1] = 20.0f;
		xIndHostPtr[1] = 1;
		xValHostPtr[1] = 200.0f;
		yHostPtr[2] = 30.0f;
		yHostPtr[3] = 40.0f;
		xIndHostPtr[2] = 3;
		xValHostPtr[2] = 400.0f;
		yHostPtr[4] = 50.0f;
		yHostPtr[5] = 60.0f;
		yHostPtr[6] = 70.0f;
		yHostPtr[7] = 80.0f;
		...
		cudaMemcpy(cooRowIndex, Pointer.to(cooRowIndexHostPtr), nnz
				* Sizeof.INT, cudaMemcpyHostToDevice);
		cudaMemcpy(cooColIndex, Pointer.to(cooColIndexHostPtr), nnz
				* Sizeof.INT, cudaMemcpyHostToDevice);
		cudaMemcpy(cooVal, Pointer.to(cooValHostPtr), nnz * Sizeof.FLOAT,
				cudaMemcpyHostToDevice);
		cudaMemcpy(y, Pointer.to(yHostPtr), 2 * n * Sizeof.FLOAT,
				cudaMemcpyHostToDevice);
		cudaMemcpy(xInd, Pointer.to(xIndHostPtr), nnz_vector * Sizeof.INT,
				cudaMemcpyHostToDevice);
		cudaMemcpy(xVal, Pointer.to(xValHostPtr), nnz_vector * Sizeof.FLOAT,
				cudaMemcpyHostToDevice);
		...
		cusparseXcoo2csr(handle, cooRowIndex, nnz, n, csrRowPtr,
				CUSPARSE_INDEX_BASE_ZERO);
		// csrRowPtr = [0 3 4 7 9]

		// Exercise Level 1 routines (scatter vector elements)
		Pointer yn = y.withByteOffset(n * Sizeof.FLOAT);
		cusparseSsctr(handle, nnz_vector, xVal, xInd, yn,
				CUSPARSE_INDEX_BASE_ZERO);
		...
		cudaMemcpy(Pointer.to(yHostPtr), y, 2 * n * Sizeof.FLOAT,
		...
		cudaMemcpy(Pointer.to(zHostPtr), z, 2 * (n + 1) * Sizeof.FLOAT,
				cudaMemcpyDeviceToHost);
		...
		if ((zHostPtr[0] != 950.0) || (zHostPtr[1] != 400.0)
				|| (zHostPtr[2] != 2550.0) || (zHostPtr[3] != 2600.0)
				|| (zHostPtr[4] != 0.0) || (zHostPtr[5] != 49300.0)
				|| (zHostPtr[6] != 15200.0) || (zHostPtr[7] != 132300.0)
				|| (zHostPtr[8] != 131200.0) || (zHostPtr[9] != 0.0)
				|| (yHostPtr[0] != 10.0) || (yHostPtr[1] != 20.0)
				|| (yHostPtr[2] != 30.0) || (yHostPtr[3] != 40.0)
				|| (yHostPtr[4] != 680.0) || (yHostPtr[5] != 760.0)
