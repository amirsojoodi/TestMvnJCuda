...
public class JCudaAsyncCopyTest {
	 ...
	 * {@link MemoryType}. This class is solely intended for testing purposes.
	 */
	static class Memory {
		/**
		...
		void put(float source[]) {
			if (type == MemoryType.DEVICE) {
				cudaMemcpy(pointer, Pointer.to(source), source.length
		...
		void get(float target[]) {
			if (type == MemoryType.DEVICE) {
				cudaMemcpy(Pointer.to(target), pointer, target.length
	...
	/**
	 * The number of float values for one chunk of data that is copied during
	 * the tests
	 */
	...
	public static void main(String args[]) {
		...
		// source memory types:
		System.out.println("Timing " + COPY_RUNS + " copy operations " + "of "
				+ NUM_FLOATS + " float values");
	...
	/**
	 * Test a synchronous (blocking) copy of the given data between the given
	 * memory types
	 * 
	 * @param targetType
	 *            The target {@link MemoryType}
	 * @param sourceType
	 *            The source {@link MemoryType}
	 * @param data
	 *            The data
	 */
	private static void testSync(MemoryType targetType, MemoryType sourceType,
			float data[]) {
	 ...
	 *            The data
	 */
	private static void testAsync(MemoryType targetType, MemoryType sourceType,
			float data[]) {
	 ...
	 *            The memory
	 * @param data
	 *            The data that is expected in the memory
	...
	private static boolean verify(Memory target, float data[]) {
		...
		for (int i = 0; i < data.length; i++) {
			...
			float f1 = result[i];
			if (f0 != f1) {
				System.out.println("ERROR: At index " + i + " expected " + f0
						+ " but found " + f1);
				passed = false;
				break;
			}
