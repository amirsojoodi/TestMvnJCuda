 ...
 * 
 * This test computes the bandwidth of the data transfer from the host to the
 * device. The host data is once allocated as pinned memory (using
 * cudaHostAlloc) and once as pageable memory (a Java array or a direct buffer,
 * comparable to 'malloc' in C).
 */
public class JCudaBandwidthTest {
	 ...
	 * 
	 * @param hostMemoryMode
	 *            The {@link HostMemoryMode}
	 * @param hostAllocFlags
	 *            The flags for cudaHostAlloc
	 */
	static void runTest(HostMemoryMode hostMemoryMode, int hostAllocFlags) {
		...
		for (int i = 0; i < count; i++) {
			...
			float bandwidth = computeBandwidth(hostMemoryMode, hostAllocFlags,
					memorySizes[i]);
	...
	/**
	 * Compute the bandwidth in MB per second for copying data from the host to
	 * the device
	 * 
	 * @param hostMemoryMode
	 *            The {@link HostMemoryMode}
	 * @param hostAllocFlags
	 *            The flags for the cudaHostAlloc call
	 * @param memorySizes
	 *            The memory sizes, in bytes
	 * @param bandwidths
	 *            Will store the bandwidth, in MB per second
	 */
	static void computeBandwidths(HostMemoryMode hostMemoryMode,
			int hostAllocFlags, int memorySizes[], float bandwidths[]) {
		for (int i = 0; i < memorySizes.length; i++) {
	 ...
	 */
	static float computeBandwidth(HostMemoryMode hostMemoryMode,
			int hostAllocFlags, int memorySize) {
		...
		final int runs = 10;
		float bandwidth = computeBandwidth(deviceData, hostData,
				cudaMemcpyHostToDevice, memorySize, runs);
	...
	/**
	 * Compute the bandwidth in MB per second for copying data from the given
	 * source pointer to the given destination pointer
	 * 
	 * @param dstData
	 ...
	 *            The source pointer
	 * @param memcopyKind
	 *            The cudaMemcpyKind. Must match the types of the source and
	 *            destination pointers!
	 * @param memSize
	 *            The memory size, in bytes
	 * @param runs
	 *            The number of times that the copying operation should be
	 *            repeated
	 * @return The bandwidth in MB per second
	 */
	static float computeBandwidth(Pointer dstData, Pointer srcData,
