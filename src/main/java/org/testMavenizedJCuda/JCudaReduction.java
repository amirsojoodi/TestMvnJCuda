...
import org.testMavenizedJCuda.JCudaRuntimeDriverMixSample;

import jcuda.*;
import jcuda.driver.*;

/**
 * Example of a reduction. It is based on the NVIDIA 'reduction' sample, and
 * uses an adopted version of one of the kernels presented in this sample.
 */
public class JCudaReduction {
	...
	public static void main(String args[]) {
		...
		for (int n = 100000; n <= 25600000; n *= 2) {
			...
			cuMemcpyHtoD(deviceInput, Pointer.to(hostInput), hostInput.length
					* Sizeof.FLOAT);
			...
			System.out.printf("  JCuda: %5.3fms, result: %f "
					+ "(copy: %5.3fms, comp: %5.3fms)\n",
					(durationCopy + durationComp) / 1e6, resultJCuda,
					durationCopy / 1e6, durationComp / 1e6);
			System.out.printf("  Java : %5.3fms, result: %f\n",
					durationJava / 1e6, resultJava);

			passed &= Math.abs(resultJCuda - resultJava) < resultJava * 1e-5;
	...
	/**
	 * Initialize the driver API and create a context for the first device, and
	 * then call {@link #prepare()}
	 */
	...
	/**
	 * Prepare everything for calling the reduction kernel function. This method
	 * assumes that a context already has been created and is current!
	 */
	...
	/**
	 * Perform a reduction on the given input, with a default number of threads
	 * and blocks, and return the result. <br />
	 * <br />
	 * This method assumes that either {@link #init()} or {@link #prepare()}
	...
	/**
	 * Perform a reduction on the given input, with the given number of threads
	 * and blocks, and return the result. <br />
	 * <br />
	 * This method assumes that either {@link #init()} or {@link #prepare()}
	 * have already been called.
	 ...
	 * @param maxBlocks
	 *            The maximum number of blocks per grid
	 * @return The reduction result
	 */
	public static float reduce(float hostInput[], int maxThreads, int maxBlocks) {
		...
		float result = reduce(deviceInput, hostInput.length, maxThreads,
				maxBlocks);
	...
	/**
	 * Performs a reduction on the given device memory with the given number of
	 * elements.
	 * 
	 * @param deviceInput
	 *            The device input memory
	 * @param numElements
	...
	/**
	 * Performs a reduction on the given device memory with the given number of
	 * elements and the specified limits for threads and blocks.
	 * 
	 * @param deviceInput
	 *            The device input memory
	 * @param numElements
	...
	public static float reduce(Pointer deviceInput, int numElements,
		...
		float result = reduce(numElements, numThreads, numBlocks, maxThreads,
				maxBlocks, deviceInput);
		return result;
	}
	 ...
	 * 
	 * @param n
	 *            The number of elements for the reduction
	 * @param numThreads
	 ...
	 * @return The reduction result
	 */
	private static float reduce(int n, int numThreads, int numBlocks,
	 ...
	 * device input memory, using the given number of threads and blocks, and
	 * write the results into the given output memory.
	 * 
	 * @param size
	 ...
	 *            The device input memory
	 * @param deviceOutput
	 *            The device output memory. Its size must at least be
	...
	private static void reduce(int size, int threads, int blocks,
		...
		Pointer kernelParameters = Pointer.to(Pointer.to(deviceInput),
				Pointer.to(deviceOutput), Pointer.to(new int[] { size }));

		// Call the kernel function.
		cuLaunchKernel(function, blocks, 1, 1, // Grid dimension
	...
	/**
	 * Compute the number of blocks that should be used for the given input size
	 * and limits
	 * 
	 * @param n
	 *            The input size
	...
	/**
	 * Compute the number of threads that should be used for the given input
	 * size and limits
	 * 
	 * @param n
	 *            The input size
	...
	private static String preparePtxFile(String cuFileName) throws IOException {
		...
		ClassLoader classLoader = JCudaReduction.class.getClassLoader();
